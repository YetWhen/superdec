...
#change this to store different testing results separately
OUTPUT_NPZ_DIR=data/shapenet_no_lm_output_npz # path to the folder where to save the output .npz files
MODEL_CHECKPOINT_DIR=checkpoints/normalized # checkpoints/normalized by default
...
python superdec/evaluate/to_npz.py checkpoints_folder="$MODEL_CHECKPOINT_DIR" output_dir="$OUTPUT_NPZ_DIR" dataset=scene scene.name="$SCENE_NAME" scene.z_up="$Z_UP"
